...
The original `SensesDevice` interface and `SensesError` remain for backward compatibility during migration.
...
4. **Use the Android module** for the physical Halo backend; it depends on `graphic-engine-halo` for BLE transport and scene compilation.
